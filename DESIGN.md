...
Cards and panels use white surfaces, 1px warm low-contrast borders, and subtle shadows. Inputs are white fields with a teal focus ring. Primary buttons are solid deep teal with white text; secondary buttons are white with a border that warms to teal on hover. Group accents are high-contrast on white: dragon red `#b3403a`, lion amber `#b45309`, wolf blue `#1d4ed8`, and penguin slate `#334155`. Wheel slices use a soft pastel palette (mint, sand, periwinkle, blush, sage) with white separators and dark Arabic labels for legibility.

## Workflow

Setup: four team panels with input fields, add buttons, and removable player chips. The default groups are التنانين, الأسود, الذئاب, and البطاريق.
