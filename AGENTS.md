...
- Sound effects use the Web Audio API in `app.js`: deceleration-matched wheel ticks plus a randomized pool of celebration sounds and a grand finale. The player wheel can additionally use `assets/sounds/spin-voice-1.mp3` from `spinClips`; `spin-voice-2.mp3` is retained only as an inactive asset and must not play. An eligible spin randomly picks between Voice 1 and synthesized ticks. Clips are fetched eagerly, decoded lazily, and always play to their natural end even after the wheel lands. The category wheel always uses ticks. A celebration voice clip pool (`celebrationClips`) joins the post-selection celebration sounds for players only, with the rule that no two mp3 sounds ever play for the same player (tracked via `spinClipUsed`). Per-player clips live in `playerClips` keyed by player name (currently `البريجي`, `حميد`, `الملا`, `حمود`, `عليوي`, `بوحمد`, and `الخلف`): when that player is chosen their clip always plays after the selection, their spin uses ticks only, and no other voice clip ever plays for them. Dedicated clips are speech-cleaned, lightly compressed, loudness-matched near `-15 LUFS`, and encoded as 48 kHz stereo 320 kbps MP3. The published `البريجي` asset is trimmed to begin at `5.1s` of the original recording. The header 🔊/🔇 toggle persists separately under `seenjeem_sound_v1` and is not cleared by `من جديد`. Sound code must never break game logic (all entry points are wrapped in try/catch).
