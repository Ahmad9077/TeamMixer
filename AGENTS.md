...
- Sound effects use the Web Audio API in `app.js`: deceleration-matched wheel ticks plus a randomized pool of celebration sounds and a grand finale. The player wheel additionally mixes in voice clips from `assets/sounds/` (see `spinClips`): each spin randomly picks between the ready clips and the synthesized ticks with equal probability. Clips are fetched eagerly, decoded lazily, faded out exactly when the wheel lands, and looped only if shorter than the spin. The category wheel always uses ticks. The header 🔊/🔇 toggle persists separately under `seenjeem_sound_v1` and is not cleared by `من جديد`. Sound code must never break game logic (all entry points are wrapped in try/catch).
