...
- Sound effects use the Web Audio API in `app.js`: deceleration-matched wheel ticks plus a randomized pool of celebration sounds and a grand finale. The player wheel additionally mixes in voice clips from `assets/sounds/` (see `spinClips`): each spin randomly picks between the ready clips and the synthesized ticks with equal probability. Clips are fetched eagerly, decoded lazily, and always play to their natural end even after the wheel lands. The category wheel always uses ticks. A celebration voice clip pool (`celebrationClips`) joins the post-selection celebration sounds for players only, with the rule that no two mp3 sounds ever play for the same player (tracked via `spinClipUsed`). Per-player clips live in `playerClips` keyed by player name (currently `البريجي`): when that player is chosen their clip always plays after the selection, their spin uses ticks only, and no other voice clip ever plays for them. The header 🔊/🔇 toggle persists separately under `seenjeem_sound_v1` and is not cleared by `من جديد`. Sound code must never break game logic (all entry points are wrapped in try/catch).
...
- `الأسود`: `بوحمد`, `الخلف`, `الهلالي`
- `الذئاب`: `حمود`, `عليوي`, `موسى`, `قرطبة`
