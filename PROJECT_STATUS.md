...
## Bright Playful Minimal Redesign (Live)
...
- The replacement was published in commit `236ac23`; GitHub Pages reached `built` for that exact commit. The public MP3 returned `HTTP 200`, matched the local SHA-256 exactly, and decoded in live Chrome with the expected duration, sample rate, and channels.
...
- On 2026-07-16, GitHub Pages reached `built` after deploying `c156652`; the public site returned `HTTP 200`, public `app.js` contains `البطاريق`, `هشوم`, `assets/sounds/player-hashoum.mp3`, `assets/sounds/player-almulla.mp3`, and `ROSTER_VERSION = 3`, and both public MP3 URLs returned `HTTP 200`.
