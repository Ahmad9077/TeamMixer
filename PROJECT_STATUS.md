...
- Added the user's third voice clip as `assets/sounds/celebration-voice-1.mp3` (7.4s): after a player is selected it joins the celebration pool as a random option, always plays to its natural end, and never plays for a player whose spin already used a spin voice clip (no two mp3 sounds for the same player). It does not play for category picks.
