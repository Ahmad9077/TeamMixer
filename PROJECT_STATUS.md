...
- Added the user's fourth voice clip as `assets/sounds/player-breiji.mp3`, dedicated to `البريجي` via the `playerClips` registry: it always plays right after he is chosen (including when he is the final player, where synthesized applause joins it instead of the standard finale), his spin uses the synthesized ticks only, and none of the other voice clips ever play for him. The published clip is trimmed to start at `5.3s` of the original 14.5-second recording.
